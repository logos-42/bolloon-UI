...
- 安装页：栏目标签框（一键脚本/npm/Android/iOS），安卓悬浮子栏已移除。Android = 本仓库 Release 直链；**当前 0.4.22 正式签名版** tag `android-v0.4.22-signed`（bolloon-0.4.22.apk / 18.29 MiB / sha256 `0c138377…` / CN=Bolloon，含手机端一键入网、微信息 x402 等 0.4.21/0.4.22 特性）；旧 `android-v0.4.20-signed`（17.88 MiB）与 debug 版 `android-v0.4.20`（CN=Android Debug）保留但已不在页面上；签名不兼容的包不可互相覆盖。iOS = 三方式 + 未签名 ipa 资产。
