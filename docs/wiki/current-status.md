...
- 安装页：栏目标签框（一键脚本/npm/Android/iOS），安卓悬浮子栏已移除。Android = **同域镜像直链** `https://bolloon.cn/dl/bolloon-0.4.22.1.apk`（18.30 MiB / sha256 `98ee32ef…` / 正式签名 CN=Bolloon / v0.4.22.1）—— 本版修了两件真机缺陷：**扫码开相机**（补 Capacitor 必需的 FileProvider(authority `${applicationId}.fileprovider`) + `<queries>IMAGE_CAPTURE</queries>`，否则静默退相册）与 **触控控制收敛成一个按钮**（原「MCP 工具(触控调用)」列表点了没反应，因 `window.__mobileTouch` 是空实现）；备用链接指向 GitHub Release tag `android-v0.4.22.1-signed`；页面还写明华为侧载风控（判风险/诈骗）时的处理（退纯净模式 / adb install）。同域镜像来自 `dl/`（gitignore，部署时由 `scripts/deploy-pages.py` 一并上传）。iOS = 三方式 + 未签名 ipa 资产。
